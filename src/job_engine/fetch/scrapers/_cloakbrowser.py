...
Helpers: ``is_enabled`` / ``require_cloakbrowser`` / ``warn_disabled`` /
``evomi_proxy_from_env``. When ``cloakbrowser`` isn't installed, the
...
def require_cloakbrowser() -> None:
    ...
    try:
        ...
        raise ScraperError(
            ...
            "Install with `pip install ats-scrapers[scrapers]`."
        ) from exc
...
def warn_disabled(scraper_name: str) -> None:
    ...
    log.warning(
        ...
        "(`pip install ats-scrapers[scrapers]`) to enable. Skipping.",
        scraper_name,
    )
